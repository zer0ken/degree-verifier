...
public class ValidateOldCommitTableModel extends DefaultTableModel {
    AppModel appModel = AppModel.getInstance();

    public ValidateOldCommitTableModel() {
    ...
    private void update() {
        setRowCount(0);
        for (List<String> row : appModel.getCommitTableValidateOld()) {
            addRow(row.toArray());
        }
    }
